...
4. Put signing secrets only in `signing`; App Store keys in the environments that need them; publisher App credentials only in `release-publishing`. Do not use `secrets: inherit`. Reusable jobs read the consumer's environment secrets. Use narrowly scoped, consumer-owned Apple keys and publisher Apps.
