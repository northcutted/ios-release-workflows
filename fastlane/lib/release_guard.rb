...
class ReleaseGuard
  ...
  def initialize(manifest, client: nil, config: nil)
    @manifest = manifest
    @client = client
    @config = config || JSON.parse(File.read(ENV.fetch("IOS_RELEASE_CONFIG")))
    raise "Release manifest schema must be 3" unless manifest["schema_version"] == 3
    raise "Wrong application" unless manifest.fetch("app").fetch("bundle_id") == @config.fetch("app_store").fetch("bundle_id") && manifest.fetch("app").fetch("team_id") == @config.fetch("team_id")
  end
  ...
  def optional_get(path)
    get(path)
  rescue RuntimeError => error
    raise unless error.message == "App Store API #{path}: HTTP 404"
    {"data" => nil}
  end

  def app_id
    @app_id ||= begin
      apps = list("/v1/apps", "filter[bundleId]" => @config.fetch("app_store").fetch("bundle_id"))
      raise "Expected one configured app" unless apps.length == 1
      apps.first.fetch("id")
    end
  end
