...
extracted from the sources, catching strings that never reached the catalog:
  ...
  xcodebuild build -scheme Application -derivedDataPath build/loc SWIFT_EMIT_LOC_STRINGS=YES ...
...
import os
ROOT = Path(os.environ.get("IOS_APP_ROOT", os.getcwd()))

DEFAULT_CATALOGS = CONFIG["localization_catalogs"]

LOCALES = CONFIG["localization_locales"]
