...
class EvidenceTests(unittest.TestCase):
    ...
    def test_assets_reject_tampering_missing_duplicate_and_traversal(self):
        file = self.root / "application.ipa"
        file.write_bytes(b"real ipa")
        record = e.asset(file)
        e.verify_assets(self.root, [record])
        for entries in ([record, record], [dict(record, name="../application.ipa")]):
    ...
    def test_wrong_provenance_identity_is_rejected(self):
        ...
        statement["predicate"]["invocation"]["environment"] = {"github_run_id": "900", "github_run_attempt": "1", "github_run_number": "100"}
        manifest = {"source_sha": SHA, "run_id": "900", "run_attempt": "1", "build_number": "100.1"}
        provenance_identity(statement, manifest)
        with self.assertRaises(ValueError): provenance_identity(statement, dict(manifest, source_sha="b" * 40))
        statement["predicate"]["invocation"]["configSource"]["entryPoint"] = ".github/workflows/pr.yml"
        with self.assertRaises(ValueError): provenance_identity(statement, manifest)
    ...
    def build_fixture(self):
        ...
        (self.root / "app-config.json").write_bytes(Path(os.environ["IOS_RELEASE_CONFIG"]).read_bytes())
        qa = {"source_sha": SHA, "checks": {name: {"status": 0, "executed": 2, "failures": 0} for name in e.CHECKS}}
        e.write(self.root / "qa-manifest.json", qa)
        inventory = {"ipa_sha256": e.digest(self.root / "application.ipa"), "applications": [
