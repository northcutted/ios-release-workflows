...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="ios-release-ipa-") as directory:
        ...
        dsym_root = Path("build/application.xcarchive/dSYMs")
        ...
        for app in applications:
            ...
            policy = next(t for t in CONFIG["targets"] if t["bundle_id"] == bundle)
            ...
            require(info.get("ITSAppUsesNonExemptEncryption") is policy["non_exempt_encryption"], "Export compliance missing from exported app")
            ...
            require(privacy.get("NSPrivacyTracking") is policy["tracking"] and "NSPrivacyAccessedAPITypes" in privacy, "Missing/invalid privacy manifest")
            ...
            for key, value in policy["entitlements"].items():
                require(entitlements.get(key) == value, f"Entitlement mismatch: {bundle} {key}")
        ...
        inventory = {"schema_version": 1, "ipa_sha256": digest(ipa), "applications": records, "embedded_components": embedded, "declared_runtime_dependencies": CONFIG["runtime_dependencies"],
