...
CHECKS = tuple(CONFIG["qa_checks"])
PRODUCER = "northcutted/ios-release-workflows"
REQUIRED = {"application.ipa", "build-env.json", "signing-env.json", "ipa-inventory.json",
            ...
            "app-store-screenshots.tar.zst", "screenshots-manifest.json", "app-config.json"}
...
def identity(data):
    require(data.get("schema_version") == 3, "Unsupported manifest schema")
...
def context():
    data = {"schema_version": 3, "repository": CONFIG["repository"], "source_ref": "refs/heads/main",
...
def verify_assets(root, entries, consumed=None):
...
def validate_manifest(root, filename, source=None, tag=None, final=False, consumed=None):
...
def build(root):
    ...
    require(inventory["ipa_sha256"] == digest(root / "application.ipa"), "Inventory identifies another IPA")
    ...
    data["candidate_id"] = f"{data['repository']}:{data['run_id']}:{data['run_attempt']}:{data['ipa_sha256']}"
